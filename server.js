...
const MINI_APP_URL = process.env.MINI_APP_URL;          // https://... (sizning serveringiz manzili)
const ADMIN_IDS = (process.env.ADMIN_IDS || "").split(",").map(s => s.trim()).filter(Boolean);
const PORT = process.env.PORT || 3000;
const REMINDER_HOUR = process.env.REMINDER_HOUR ? Number(process.env.REMINDER_HOUR) : null; // 0-23, masalan 18
const DEV_BYPASS = process.env.DEV_BYPASS === "1";

if (!BOT_TOKEN) { console.error("❌ BOT_TOKEN .env faylda ko‘rsatilmagan."); process.exit(1); }
if (!MINI_APP_URL) console.warn("⚠️  MINI_APP_URL ko‘rsatilmagan — web_app tugmasi ishlamaydi.");

/* --------------------------- initData tekshiruvi --------------------------- */
// Telegram WebApp initData'ni bot token bilan HMAC orqali tasdiqlaydi.
...
const adminOnly = (req, res, next) =>
  req.isAdmin ? next() : res.status(403).json({ error: "Bu bo‘lim faqat administrator uchun." });
...
bot.command("start", async (ctx) => {
  ...
  await ctx.reply(
    ...
    "Har kuni quyidagi tugma orqali bo‘lim hisobotingizni topshiring. " +
    "Administratorlar barcha hisobotlarni admin panelda ko‘radi.",
